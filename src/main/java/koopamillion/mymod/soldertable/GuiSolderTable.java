...
public class GuiSolderTable extends GuiContainer {
  ...
  //  private int j = 0;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (mouseX > guiLeft + 9 && mouseX < guiLeft + 26 && mouseY > guiTop + 10 + increase && mouseY < guiTop + 56 + increase) {
            if(solder.getClientFluidAmount() == 0){
                drawHoveringText(Collections.singletonList("0mb"), mouseX, mouseY, fontRenderer);

            }else{
                drawHoveringText(Collections.singletonList(solder.getClientFluidAmount() + "mb of "+ new TextComponentTranslation(tank.readFromNBT(solder.getClienttag().getCompoundTag("tank")).getFluid().getFluid().getUnlocalizedName(), new Object[0]).getFormattedText()), mouseX, mouseY, fontRenderer);}
        }
    ...
    private void drawFluidBar(float fluidamount, int capacity2) {
        ...
        if(tank.readFromNBT(solder.getClienttag().getCompoundTag("tank")).getFluid() != null){
            texture = tank.readFromNBT(solder.getClienttag().getCompoundTag("tank")).getFluid().getFluid().getBlock().getDefaultState();

        }else{
            texture = ModBlocks.blockSolderFluid.getDefaultState();
        }

        TextureAtlasSprite sprite = this.mc.getBlockRendererDispatcher().getBlockModelShapes().getTexture(texture);
